# Consumer instance for Channels layers
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if typ == "message":
            await self.channel_layer.group_send(
                ...
                {
                    "type": "chat_message",
                    "username": self.scope["user"].get_full_name(), "message": content["message"],
                },
            )
        elif typ == "heartbeat":
...
class ChatNotifyConsumer(AsyncHttpConsumer):
    ...
    async def handle(self, body):
        is_emplee = await database_sync_to_async(self.is_employee_func)(self.scope["user"])
        if is_emplee:
